...
export default async function handlerGenerateImage(req, res) {
  ...
  try {
    ...
    const finalPrompt = `Create a high-quality image based on this description: Artistic Style/Theme: ${settings.style}. Character/Outfit: ${settings.charPrompt}. Context/Action: ${settings.contextPrompt}. Reference Instruction: ${accuracyInstruction} Ensure the lighting and composition are professional.`;
    ...
    for (let i = 0; i < numImages; i++) {
      requests.push(ai.models.generateContent({ model: modelName, contents: { parts }, config: config }));
    }
