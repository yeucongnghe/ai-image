...
export default async function handler(req, res) {
  // Thiết lập Header để luôn trả về JSON, tránh lỗi "Unexpected token A"
  ...
  try {
    ...
    const prompt = `Remove the background of this image. Keep the main subject with ${accuracy}% precision.`;
    ...
    if (!imageData) {
      throw new Error("AI không trả về dữ liệu ảnh. Có thể ảnh vi phạm chính sách hoặc quá mờ.");
    }

    return res.status(200).json({ images: [imageData.inlineData.data] });

  } catch (error) {
    console.error("LOG LỖI CHI TIẾT:", error.message);
