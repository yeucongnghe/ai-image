  ...
  try {
    ...
    if (data.images && data.images.length > 0) {
      const processedImages = data.images.map((img: string) => {
        // 1. Tìm vị trí dấu phẩy cuối cùng (ngăn cách giữa tiền tố và dữ liệu)
        const lastCommaIndex = img.lastIndexOf(",");
        
        // 2. Lấy PHẦN ĐUÔI (Mã Base64 thô)
        // Nếu có dấu phẩy, lấy sau dấu phẩy. Nếu không, lấy cả chuỗi.
        const rawBase64 = lastCommaIndex !== -1 
          ? img.substring(lastCommaIndex + 1) 
          : img;

        // 3. TRẢ VỀ MÃ THÔ (Không tự ý thêm "data:image..." ở đây)
        // Để ResultViewer của Studio tự lo phần tiền tố
