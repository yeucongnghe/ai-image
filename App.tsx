...
const handleGenerate = async () => {
  ...
  try {
    ...
    // Sửa điều kiện kiểm tra vì Backend chỉ trả về { images: [...] } 
    if (data.images && data.images.length > 0) {
      let base64String = data.images[0];
      
      // 1. Kiểm tra và loại bỏ tiền tố bị lặp nếu có
      if (base64String.startsWith('data:image/png;base64,data:image/png;base64,')) {
        base64String = base64String.replace('data:image/png;base64,data:image/png;base64,', 'data:image/png;base64,');
      } 
      // 2. Nếu chưa có tiền tố nào thì mới thêm vào một lần duy nhất
      else if (!base64String.startsWith('data:image')) {
        base64String = `data:image/png;base64,${base64String}`;
      }

      // Cập nhật vào State
      setResultImages(base64String); 
    } else {
