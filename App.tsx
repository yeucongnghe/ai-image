...
/* const handleGenerate = async () => {
  ...
  if (!refImage && !refImage2 && !settings.charPrompt?.trim()) {
  ...
  try {
    // Hàm chuyển file sang Base64 để giữ nguyên chất lượng khi gửi qua API
    ...
    // Chuẩn bị payload
    const payload = {
      image1: refImage ? await toBase64(refImage) : null,
      image2: refImage2 ? await toBase64(refImage2) : null,
      //prompt: settings.charPrompt,
      config: settings // Gửi toàn bộ settings chuẩn từ Studio
    };

    // Gọi đến Backend của bạn
    const response = await fetch('/api/generate-image', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    ...
    alert("Lỗi tạo ảnh: " + (error.message || "Không thể kết nối server"));
  } finally {
    setIsProcessing(false);
  }
}; */
...
/*  const handleGenerate = async () => {
    ...
    try {
      ...
      alert("Lỗi tạo ảnh: " + (error.message || JSON.stringify(error)));
    } finally {
      setIsProcessing(false);
    }
  }; */
