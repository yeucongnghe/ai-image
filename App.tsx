...
const App: React.FC = () => {
  ...
  const handleDeleteScene = (id: string) => {
    // Clean up regenerating state if present
    ...
    // Delete scene immediately without confirmation for better UX
    setScenes(prev => {
      const filtered = prev.filter(s => s.id !== id);
      // Renumber scenes to ensure sequence is correct (1, 2, 3...)
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-gray-900 overflow-hidden">
      ...
      {/* GLOBAL HEADER */}
      <header className="h-16 bg-[#482201] border-b border-gray-800 flex items-center justify-center flex-col shrink-0 z-50 shadow-md">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-2/3 h-full flex flex-col bg-[#060b3b]">
          
          {/* Scene List Area */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[#060b3b] pb-20">
            
            {/* Header moved inside scroll area */}
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3">
                {scenes.length > 0 && (
                    ...
                    >
                      <Download className="w-4 h-4" /> Kết xuất ra file Text
                    </button>
                )}
              </div>
            </div>
            ...
            {scenes.length === 0 ? (
              <div className="h-[70vh] flex flex-col items-center justify-center text-gray-600 space-y-4 opacity-70">
                <Layers className="w-16 h-16 stroke-1" />
                <p className="text-lg">Chưa có cảnh nào. Hãy nhập mô tả tóm tắt câu chuyện bên trái rồi nhấn Tạo danh sách cảnh.</p>
              </div>
            ) : (
              scenes.map((scene) => (
                <SceneCard 
