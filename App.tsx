...
const handleGenerate = async () => {
  ...
  try {
    ...
    if (data.images && data.images.length > 0) {
      const processedImages = data.images.map((base64Raw: string) => {
        // 1. Loại bỏ tất cả các tiền tố cũ nếu có để lấy chuỗi gốc
        const pureBase64 = base64Raw.replace(/data:image\/[a-z]+;base64,/g, "");
        
        // 2. Chỉ thêm lại đúng 1 lần tiền tố chuẩn
        return `data:image/png;base64,${pureBase64}`;
      });

      // 3. Cập nhật mảng ảnh sạch vào State
