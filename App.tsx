...
const handleGenerate = async () => {
  ...
  try {
    ...
    if (data.images && data.images.length > 0) {
      // .map() sẽ duyệt qua từng phần tử và trả về một MẢNG MỚI
      const processedImages = data.images.map((img: string) => {
      // Tìm vị trí dấu phẩy cuối cùng để loại bỏ mọi lớp tiền tố bị lặp
      const lastCommaIndex = img.lastIndexOf(",");
      const pureBase64 = lastCommaIndex !== -1 
      ? img.substring(lastCommaIndex + 1) 
      : img;
      // Trả về chuỗi sạch với đúng 1 tiền tố chuẩn
      return `data:image/png;base64,${pureBase64.trim()}`;
    });
    // Truyền mảng mới này vào State (KHÔNG thêm ngoặc vuông lồng bên ngoài)
    setResultImages(processedImages);
    } else {
                  ...
                  <div className={`relative w-full h-48 border-2 border-dashed rounded-xl flex flex-col items-center justify-center transition-all duration-300 cursor-pointer overflow-hidden group ${refImage ? 'border-brand-500 bg-slate-900' : 'border-slate-700 hover:border-brand-400 hover:bg-slate-800 hover:shadow-lg hover:shadow-brand-500/10'}`} onClick={() => fileInputRef.current?.click()}>
                    {refImage ? (
                      <>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setRefImage(null); }} className="absolute top-2 right-2 p-1.5 bg-slate-900/60 hover:bg-red-500 text-white/80 hover:text-white rounded-full transition-all duration-200 border border-white/10 shadow-sm z-10"><X className="w-4 h-4" /></button>
                      </>
                    ) : (
                  ...
                  <div className={`relative w-full h-48 border-2 border-dashed rounded-xl flex flex-col items-center justify-center transition-all duration-300 cursor-pointer overflow-hidden group ${refImage2 ? 'border-brand-500 bg-slate-900' : 'border-slate-700 hover:border-brand-400 hover:bg-slate-800 hover:shadow-lg hover:shadow-brand-500/10'}`} onClick={() => fileInputRef2.current?.click()}>
                    {refImage2 ? (
                      <>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setRefImage2(null); }} className="absolute top-2 right-2 p-1.5 bg-slate-900/60 hover:bg-red-500 text-white/80 hover:text-white rounded-full transition-all duration-200 border border-white/10 shadow-sm z-10"><X className="w-4 h-4" /></button>
                      </>
                    ) : (
                      <div className="text-center p-2 transform group-hover:scale-105 transition-transform duration-300">
