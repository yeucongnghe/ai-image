...
export const removeBackgroundAI = async (base64Image: string): Promise<string> => {
  const apiKey = "AIzaSyBJq2MRdqCayWAAnJhhLx_Pin-0qq1L0SQ" //process.env.NEXT_PUBLIC_GEMINI_API_KEY || process.env.API_KEY || '';
  if (!apiKey) {
    console.error("Lỗi: Không tìm thấy API Key via NEXT_PUBLIC_GEMINI_API_KEY");
    throw new Error("Chưa cấu hình đúng API Key cho Website.");
  }
...
export const generateImageAI = async (
  ...
  if (accuracyLevel >= 80) {
    accuracyInstruction = "CRITICAL: Preserve the facial features and body structure of the reference image(s) with EXTREME fidelity. The output must look exactly like the person in the reference.";
  } else if (accuracyLevel >= 40) {
    accuracyInstruction = "Use the reference image(s) as a strong guide for the character's general look and facial structure, but allow for moderate artistic variations to fit the style.";
  } else {
    accuracyInstruction = "Use the reference image(s) only as a loose inspiration. You are free to significantly alter the face and body structure to create a creative re-interpretation.";
  }
  ...
  // Add images to parts. Note: Order matters, usually putting images first helps.
  // We unshift in reverse order so Image 1 is first, Image 2 is second.
  
  if (base64RefImage2) {
    parts.unshift({
      inlineData: {
        mimeType: 'image/png',
        data: base64RefImage2
      }
    });
  }
